...
$id= $_POST['consolidado'];


        $queryconsolidados = mysqli_query($conn,"SELECT * FROM consolidados WHERE id ='$id[0]'");
        $comprobacion_consolidado = mysqli_num_rows($queryconsolidados);

        if ($comprobacion_consolidado > 0) {
                
        $mostrar=mysqli_fetch_array($queryconsolidados);
                ...
                $querynuevos = mysqli_query($conn,"DELETE FROM consolidados WHERE id ='$id[$i]'");
                }
...
<body>
	...
	<?php	
	     ...
	     $_SESSION['consolidado'] = $id[0];

             $queryministerios = mysqli_query($conn,"SELECT * FROM ministerios");
             $comprobacion_ministerios = mysqli_num_rows($queryministerios);

             $queryministerio = mysqli_query($conn,"SELECT * FROM consolidados INNER JOIN ministerio_asignado ON consolidados.id= ministerio_asignado.id_usuario INNER JOIN ministerios ON ministerios.id = ministerio_asignado.id_ministerio  WHERE consolidados.id= $id[0]");
             $vfministerio= mysqli_num_rows($queryministerio);

	     $queryhcm = mysqli_query($conn,"SELECT * FROM hcm");
             $comprobacion_hcm = mysqli_num_rows($queryhcm);
	     $queryhcmusu = mysqli_query($conn,"SELECT * FROM consolidados INNER JOIN hcm_asignado_consolidado ON consolidados.id= hcm_asignado_consolidado.id_usuario INNER JOIN hcm ON hcm.id = hcm_asignado_consolidado.id_hcm  WHERE consolidados.id= $id[0]");
...
<form method="post" action="consolidado_editar.php">
<h1 align="center">Editar</h1>
	...
	<label for="">Nombre:</label>
	 <input type="text" name="nombre" pattern="[A-Za-z ]+" title="Solo se permiten letras" value="<?php echo $mostrar['nombres']; ?>" required>
    </div>

     <div class="form-group">
	 <label for="">Apellido:</label>
	 <input type="text" name="apellido" pattern="[A-Za-z ]+" title="Solo se permiten letras" value="<?php echo $mostrar['apellidos']; ?>" required>
     </div>
	 ...
	 <input type="text" minlength="7" maxlength="8" name="cedula" value="<?php echo $mostrar['cedula']; ?>" required>
     </div>


     <div class="form-group">
	 <label for="">Telefono:</label>
	  <input type="tel" name="telefono" pattern="^\+?[0-9 ]{7,15}$" minlength="11" maxlength="15" value="<?php echo $mostrar['telefono']; ?>" required>
     </div>
...
<select id="profesiones" name="profesiones" required>
  ...
  <option value="Vestuarista" <?php if ($mostrar['profesion'] == "Vestuarista") { echo "selected"; } ?>>Vestuarista</option>
       ...
       if ($vfministerio > 0) {
        ...
        <div class="form-group">
        <label for="">Ministerios actuales que puedes eliminar (opcional)</label>
        <select name="ministerioeli[]" multiple>
                <?php
                $o=0;
                while($mostrar3=mysqli_fetch_array($queryministerio)){ 
                $ministerio[$o] = $mostrar3['id'];        
                ?>
        ...
        <div class="form-group">
        <label>Añadir Ministerio (opcional)</label>
          <select name="txtministerio[]" multiple>
	  ...
	  <input type="text" name="invitado" pattern="[A-Za-z ]+" title="Solo se permiten letras" value="<?php echo $mostrar['invitado_por']; ?>"  required>
      </div>
